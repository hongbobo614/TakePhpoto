...
public class FunctionConfig implements Serializable {
    ...
    private boolean enableCrop= false; // 是否裁剪图片，只针对单选图片有效 默认fasle
    private boolean isPreviewVideo=false; // 是否可预览视频(播放)
    private int imageSpanCount = 3; // 列表每行显示个数
    ...
    private boolean isCheckNumMode = true;// 是否显示QQ风格选择图片
